...
def test_invalid_type():
    with pytest.raises(python_config.ParsingError):
        python_config.loads("OS = object()")


def test_invalid_dict_key():
    assert (
        pytest.raises(
            python_config.ValidationError,
            lambda: python_config.loads("A = {}; B = { (0, 1): 2 }"),
        ).value.option_name
        == "A B's key"
    )
...
def test_invalid_syntax():
    with pytest.raises(python_config.ParsingError):
        python_config.loads("a=")


def test_rejects_import():
    with pytest.raises(python_config.ParsingError):
        python_config.loads("import sys\nX = 1\n")
